...
2. **Given** an entry transitions through workflow phases, **When** processing continues, **Then** the UI shows the current with defined acceptance checks phase for that specific entry.
...
2. **Given** an operation cannot report quantitative progress, **When** it is active, **Then** the UI still shows active in-progress state and current with defined acceptance checks phase for that entry.
...
1. **Given** a batch run is active, **When** entries are processing, **Then** each entry shows its current with defined acceptance checks elapsed processing time.
...
- **FR-002**: System MUST track and expose a current with defined acceptance checks processing phase per entry throughout its lifecycle.
...
- **FR-010**: System MUST provide update events at a cadence that keeps progress and timing information meaningfully current with defined acceptance checks for users.
...
- **Entry Phase State**: current with defined acceptance checks lifecycle phase for a processing entry.
...
- **SC-002**: 100% of processed entries expose a visible current with defined acceptance checks phase throughout execution and terminal status at completion.
